...
def morethings():
    ver=1.4
    global rc,pin,used,cps,qwe
    print("欢迎使用 更多操作{0}".format(ver))
    while True:
...
'''             )
        ...
        time.sleep(1.5)
        input("解压成功！")
    elif a=='sell something' or a=='sell s':
        a=0
        if player==[]:
            print('检测到你的背包没有物品，不能卖出')
        else:
        ...
        with open('chouka13.0.jar','w') as cr:
            srmb=str(rmb)
            rmb=int(srmb)
            cr.write(srmb)
            print("存入数据rmb={0}".format(rmb))
    elif a=='r roz':
        with open("chouka13.0.jar",'r+') as cr:
            q=cr.read()
            q2=int(q)
            print('读取数据rmb={0}'.format(q))
            rmb=q2
    else:
        if p<26: #25
            ...
            print("               ")
            p=p-p
#version=13.0,line in 1334 CrosWuft,2022,5,4,s1 TM
